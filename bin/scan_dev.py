...
novex/sources/* возвращает CollectResult, который складывается в общие
таблицы blocks/flats/snapshots с глобально-уникальными id.

  python -m bin.scan_dev --db data/novex.db --developer "ГК ФСК"
  python -m bin.scan_dev --db data/novex.db --developer "ПИК"
  python -m bin.scan_dev --db data/novex.db --all           # все 10 параллельно

До 2026-05-25 был отдельный bin/scan.py для PIK и bin/scan_dev.py для
не-PIK-источников в общую БД.
"""
...
from novex.developers import DEVELOPERS, ID_NAMESPACE
from novex.quality import DataQualityStats
...
from novex.sources import pik as pik_source
from novex.sources.base import CollectResult, SourceError, build_rows
from novex.store import apply_schema, record_scan_run, refresh_materialized, upsert
...
def _pik_collect_for_known_blocks(db_path: Path) -> CollectResult:
    ...
    """
    log = logging.getLogger("novex.scan_dev")
...
# Имя каждого источника обязано быть в реестре novex.developers — иначе
...
assert not _UNKNOWN_SOURCES, (
    f"источники вне реестра застройщиков novex.developers: {_UNKNOWN_SOURCES}"
)
...
def run_sweep(
    ...
    # scan_dev стартует ИЗ novex-scan.service через OnSuccess, т.е. этот refresh
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--db", default="data/novex.db", type=Path)
    parser.add_argument(
        "--developer",
