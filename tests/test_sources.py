...
from novex.developers import ID_NAMESPACE, namespaced_id
from novex.sources import fsk
from novex.sources.base import (
    CollectResult,
    NormBlock,
    NormFlat,
    _detect_discount,
...
from novex.store import apply_schema
...
def test_to_global_id_rehashes_out_of_range_numeric(caplog):
    ...
    from novex.developers import split_id
...
def test_fsk_collect_aggregates_floors_max_from_floor_numbers(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.fsk.request_json", fake)
    result = fsk.collect()
...
def test_absolut_collect_aggregates_floors_from_buildingFloor(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.absolut.request_json",
...
def test_fsk_collect_with_mocked_api(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.fsk.request_json", fake_request_json)
    result = fsk.collect()
...
def test_fsk_rows_apply_to_schema(monkeypatch):
    ...
    apply_schema(conn)
    from novex.blocks_meta import upsert_block_meta
    from novex.store import upsert
    for bp in blocks:
...
from novex.sources import donstroy  # noqa: E402
...
def test_donstroy_collect_aggregates_floors_total_per_block(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.donstroy.request_json",
...
def test_donstroy_collect_paginates_and_dedups_blocks(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.donstroy.request_json", fake_request_json)
    result = donstroy.collect()
...
from novex.sources import a101  # noqa: E402
...
def test_a101_collect_aggregates_floors_max_per_block(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.a101.request_json",
                        lambda *a, **k: pages.pop(0))
    result = a101.collect()
...
def test_a101_collect_follows_next_pagination(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.a101.request_json", fake_request_json)
    result = a101.collect()
...
from novex.sources import level  # noqa: E402
...
def test_level_collect_aggregates_floors_section_total_per_block(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.level.request_json",
                        lambda *a, **k: pages.pop(0))
...
from novex.sources import absolut  # noqa: E402
...
def test_absolut_collect_cursor_pagination(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.absolut.request_json", fake_request_json)
    result = absolut.collect()
...
def test_granel_project_meta_detects_non_msk_by_coords(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.granel.request_json",
        lambda *a, **k: {"results": [
...
def test_granel_collect_follows_next_pagination(monkeypatch):
    ...
    def fake_request_json(session, method, url, **kw):
        ...
        return pages.pop(0)
    monkeypatch.setattr("novex.sources.granel.request_json", fake_request_json)
...
def test_granel_collect_upgrades_next_http_to_https(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.granel.request_json", fake)
    granel.collect()
...
from novex.sources import ingrad  # noqa: E402
...
def test_ingrad_second_house_fills_missing_coords(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.ingrad.request_json",
                        lambda *a, **k: pages.pop(0))
...
from novex.sources import brusnika  # noqa: E402
...
def test_brusnika_collect_region_prefixes_slug(monkeypatch):
    ...
    monkeypatch.setattr("novex.sources.brusnika.request_json",
...
def test_brusnika_region_failure_doesnt_break_others(monkeypatch):
    ...
    from novex.sources.base import SourceError as _SE
    def fake(session, region, city):
        if region == "moskva":
            raise _SE("boom")
        return CollectResult(blocks=[NormBlock(native_id="x", name="X", slug="x")],
                             flats=[])
    monkeypatch.setattr("novex.sources.brusnika._collect_region", fake)
    r = brusnika.collect()
...
from novex.sources import pik as pik_src  # noqa: E402
