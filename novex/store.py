...
# Зеркало novex.geo._CITY_PATTERNS / city_from_address. Держим в SQL чтобы не
...
def _create_views(conn: sqlite3.Connection) -> None:
    ...
    завершения это VIEW — иначе между scan-стартом и scan-концом /novex/today_all
...
def apply_schema(conn: sqlite3.Connection) -> None:
    ...
    _migrate_flats(conn)
    sql = files("novex").joinpath("schema.sql").read_text(encoding="utf-8")
