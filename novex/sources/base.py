...
с глобально-уникальными id (см. novex.developers).
"""
...
from novex import PikParserError
from novex.developers import ID_NAMESPACE, namespaced_id, stable_int_id
from novex.geo import CITY_CENTERS, city_from_address, haversine_km
from novex.quality import GEO_MAX_KM, DataQualityStats, geo_ok, price_ok

log = logging.getLogger("novex.sources")
...
def build_rows(
    ...
    *,
    ...
    Data-quality gate (см. novex.quality): ЖК с координатами дальше GEO_MAX_KM
    ...
    for f in result.flats:
        ...
        if block_gid not in known_block_ids:
            skipped_orphan += 1
            continue
        # цена 0/NULL/вне вилки — логический мусор, не пишем (см. novex.quality)
